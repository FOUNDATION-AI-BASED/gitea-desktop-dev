...
const envEndpoint = process.env['DESKTOP_GITEA_API_ENDPOINT']
const envHTMLURL = process.env['DESKTOP_GITEA_HTML_URL']
const envAdditionalCookies =
  process.env['DESKTOP_GITEA_ADDITIONAL_COOKIES']
...
if (!ClientID || !ClientID.length || !ClientSecret || !ClientSecret.length) {
  log.warn(
    `DESKTOP_OAUTH_CLIENT_ID and/or DESKTOP_OAUTH_SECRET is undefined. OAuth sign-in will be disabled. Use Personal Access Token sign-in, or create an OAuth app in your Gitea: Settings → Applications → Create OAuth2 Application, then set the env vars.`
  )
}

export type GitHubAccountType = 'User' | 'Organization'

/** The OAuth scopes we want to request (Gitea compatible) */
const oauthScopes = ['read:repository', 'write:repository', 'read:user', 'write:user']
...
export class API {
  ...
  private async ghRequest(
    ...
    // Consider invalid token when the status is 401 (Gitea doesn't send
    // X-GitHub-Request-Id, so we check for 401 only)
    if (
      response.status === HttpStatusCode.Unauthorized &&
      !response.headers.has('X-Gitea-OTP')
    ) {
 ...
 *
 * https://gitea.example.com/org/repo -> https://gitea.example.com/api/v1
 */
export function getEndpointForRepository(url: string): string {
  const parsed = URL.parse(url)
  return `${parsed.protocol}//${parsed.hostname}/api/v1`
}
...
/**
 * Get the Gitea API URL for an HTML URL. For example:
 *
 * http://gitea.example.com -> https://gitea.example.com/api/v1
 */
export function getGiteaAPIURL(endpoint: string): string {
  const { host, protocol } = new window.URL(endpoint)
  return `${protocol}//${host}/api/v1`
}

/** Alias for backwards compatibility */
export const getEnterpriseAPIURL = getGiteaAPIURL

export const getAPIEndpoint = (endpoint: string) => endpoint

/** Sentinel endpoint - no default Gitea server, used for anonymous accounts */
export function getPlaceholderEndpoint(): string {
...
/** @deprecated Use getPlaceholderEndpoint. Kept for compatibility. */
export const getDotComAPIEndpoint = getPlaceholderEndpoint
...
function tryUpdateEndpointVersionFromResponse(
  ...
  const version =
    response.headers.get('x-gitea-version') ??
    response.headers.get('x-github-enterprise-version')
  if (version !== null) {
    updateEndpointVersion(endpoint, version)
  }
}
...
/**
 * Attempts to determine whether or not the url belongs to a Gitea host.
 *
 * This is a best-effort attempt and may return `undefined` if encountering
 * an error making the discovery request
 */
export async function isGiteaHost(url: string) {
  ...
  const metaUrl = `${endpoint}/version?gitea=${crypto.randomUUID()}`
  ...
  try {
    ...
    log.debug(`isGiteaHost: failed with endpoint ${endpoint}`, e)
    return undefined
  } finally {
...
/** @deprecated Use isGiteaHost */
export const isGitHubHost = isGiteaHost
