...
export class SignInStore extends TypedBaseStore<SignInState | null> {
  ...
  /**
   * @deprecated For Gitea, use beginEnterpriseSignIn. Redirects to endpoint entry.
   */
  public beginDotComSignIn(resultCallback?: (result: SignInResult) => void) {
    this.beginEnterpriseSignIn(resultCallback)
  }
  ...
  public async authenticateWithToken(endpoint: string, token: string) {
    const currentState = this.state
    ...
    try {
      ...
      log.warn('[SignInStore] token authentication failed', e)
      this.setState({
        ...currentState,
        error: new Error(
          'Invalid token. Please check your token and try again. Create one in Gitea: Settings → Applications → Generate New Token.'
        ),
        loading: false,
      })
    }
  ...
  /**
   * Initiate a sign in flow for a Gitea instance.
  ...
  public async setEndpoint(url: string): Promise<void> {
    ...
    try {
      ...
      if (e.name === InvalidURLErrorName) {
        error = new Error(
          `The Gitea instance address doesn't appear to be a valid URL. We're expecting something like https://gitea.example.com.`
        )
